...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: 'tg_confirm',
            description: 'Ask the user for yes/no confirmation via Telegram (Approve / Deny buttons). ' +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    options: {
                        type: 'array',
                        items: { type: 'string' },
                        description: 'Answer choices shown as inline buttons. Omit for free-text response.',
                    },
                    timeout_seconds: {
                        type: 'number',
                        description: 'Max wait time in seconds (default: 300)',
        ...
        {
            name: 'tg_inbox',
            description: 'List durable Telegram tasks without removing them. ' +
        ...
        {
            name: 'tg_ack',
            description: 'Acknowledge one or more inbox tasks. New tasks become accepted and remain durable.',
            inputSchema: {
                type: 'object',
                properties: {
                    task_id: {
        ...
        {
            name: 'tg_progress',
            description: 'Update task progress and edit one persistent Telegram status message. ' +
        ...
        {
            name: 'tg_complete',
            description: 'Complete a task, update its status message, and optionally send multiple result files.',
            inputSchema: {
                type: 'object',
                properties: {
                    task_id: { type: 'string', description: 'Task ID' },
        ...
        {
            name: 'tg_fail',
            description: 'Mark a task as failed and show Retry/Cancel controls in Telegram.',
            inputSchema: {
                type: 'object',
                properties: {
                    task_id: { type: 'string', description: 'Task ID' },
        ...
        {
            name: 'tg_cancelled',
            description: 'Check whether cancellation was requested for a task. ' +
        ...
        {
            name: 'tg_get_tasks',
            description: 'Legacy compatibility tool. Retrieves new tasks and marks them accepted. ' +
        ...
        {
            name: 'tg_send_file',
            description: 'Send a local file (archive, log, PDF, or image) to the user via Telegram. ' +
        ...
        {
            name: 'tg_send_files',
            description: 'Send multiple local files in one call. Consecutive images or documents ' +
        ...
        },
        {
            ...
            description: 'Open a URL (e.g. local web app dev server, document, page) in headless Chromium, ' +
    ...
    try {
        switch (name) {
            ...
            case 'tg_ask': {
                ...
                const acceptFiles = args?.accept_files ?? false;
                const answer = await telegram.ask(question, options, timeoutMs, taskId, acceptFiles);
            ...
            // ── tg_ack ─────────────────────────────────────────────────────────
            case 'tg_ack': {
                const singleId = args?.task_id;
                const multipleIds = args?.task_ids;
            ...
            case 'tg_fail': {
                ...
                const task = await telegram.failTask(taskId, error, args.retryable);
                return { content: [{ type: 'text', text: JSON.stringify(task, null, 2) }] };
            }
            // ── tg_cancelled ───────────────────────────────────────────────────
            case 'tg_cancelled': {
                ...
                if (!taskId)
                    throw new Error('`task_id` is required');
                const markCancelled = args?.mark_cancelled ?? false;
