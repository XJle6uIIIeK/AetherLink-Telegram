...
export class TelegramBridge {
    ...
    geminiApiKey = null;
    // File Browser state
    waitingForFileEdit = null;
    pathIdMap = new Map(); // id -> absolutePath
    pathIdCounter = 0;
    ...
    createIncomingTask(input) {
        return this.taskStore.create(input);
    }
    findPendingResponseRequest(replyToMessageId, responseType) {
        ...
        const matchingRequests = [...this.pendingRequests.entries()]
            .filter(([, pending]) => (responseType === 'text' ? pending.acceptText : pending.acceptFiles));
        return matchingRequests.length === 1 ? matchingRequests[0][0] : null;
    }
    resolveIncomingResponse(value, replyToMessageId, responseType) {
    ...
    queuePhotoAlbum(mediaGroupId, chatId, item) {
    ...
    async flushPhotoAlbum(mediaGroupId) {
        const pendingAlbum = this.pendingPhotoAlbums.get(mediaGroupId);
        if (!pendingAlbum)
            return;
        clearTimeout(pendingAlbum.timeout);
        ...
        try {
            const loadingMsg = await this.bot.api.sendMessage(pendingAlbum.chatId, `📸 *Downloading ${pendingAlbum.items.length} photos...*`, { parse_mode: 'Markdown' });
            loadingMessageId = loadingMsg.message_id;
        }
        catch (err) {
            console.error(`[TG] Failed to send photo album progress message (${mediaGroupId}):`, err);
        }
        await this.processPhotoUploads(pendingAlbum.chatId, pendingAlbum.items, loadingMessageId, mediaGroupId);
    }
    async processPhotoUploads(chatId, items, loadingMessageId, mediaGroupId) {
        try {
            ...
            const absolutePaths = await Promise.all(orderedItems.map(async (item, index) => {
            ...
            const captions = [...new Set(orderedItems
            ...
            const deliveredToPendingRequest = this.resolveIncomingResponse(taskText, orderedItems[0]?.replyToMessageId, 'file');
            ...
            await this.updatePhotoStatus(chatId, loadingMessageId, `📸 *${count === 1 ? 'Photo' : `${count} photos`} received & saved!*\n${savedPaths}\n${deliveryStatus}`);
        }
        catch (err) {
            const errorMsg = err instanceof Error ? err.message : String(err);
            await this.updatePhotoStatus(chatId, loadingMessageId, `⚠️ *Failed to download ${items.length === 1 ? 'photo' : 'photo album'}:*\n\`${errorMsg}\``);
        }
    }
    async updatePhotoStatus(chatId, loadingMessageId, text) {
    ...
    async upsertTaskStatus(task, text, keyboard) {
        const chatId = task.telegramChatId ?? this.chatId;
        if (!chatId)
            throw new Error('Task has no Telegram chat ID');
    ...
    setupHandlers() {
        ...
        this.bot.use(async (ctx, next) => {
            ...
            if (this.chatId === null) {
                ...
                if (senderId && this.pairingKey && (!this.pairingExpiresAt || Date.now() <= this.pairingExpiresAt) && suppliedKey === this.pairingKey) {
                    ...
                    console.error('[Security] Telegram owner paired successfully');
                    await ctx.reply('✅ AetherLink подключён. Управление выполняется через MCP.');
                    return;
                }
                await ctx.reply("🔒 *AetherLink is not paired.*\nUse the private pairing button generated by the plugin setup window.", { parse_mode: 'Markdown' });
                return;
            }
        ...
        // Slash commands are intentionally not exposed. The only accepted slash
        // message is the one-time /start <pairing-key> consumed by the security
        // middleware above before the bot is paired.
        ...
        this.bot.on('callback_query:data', async (ctx) => {
            ...
            if (data === 'menu:tasks') {
                await ctx.answerCallbackQuery();
                const tasks = this.taskStore.list(['new', 'accepted', 'running', 'waiting_user', 'failed', 'cancel_requested'], 20);
                if (tasks.length === 0) {
                    await ctx.reply('📝 Queue is empty.');
                }
                else {
                    const list = tasks
                        .map((task, i) => (`${i + 1}. [${task.status}] \`${task.id}\` ${task.text}`))
                        .join('\n');
            ...
            if (data.startsWith('kill_proc:')) {
                ...
                if (proc) {
                    ...
                    try {
                        await this.bot.api.editMessageText(this.chatId, proc.messageId, `🛑 *Command cancelled:* \`${proc.command}\``, { parse_mode: 'Markdown' });
                    }
                    catch {
                        // ignore
                    }
            ...
            if (data.startsWith('fb_agent:')) {
                ...
                if (targetPath) {
                    ...
                    await ctx.reply(`🤖 File sent to agent: \`${path.basename(absolutePath)}\`\nTask: \`${task.id}\``, { parse_mode: 'Markdown' });
                }
                else {
                    await ctx.answerCallbackQuery({ text: '⚠️ Path expired or not found.' });
                }
            ...
            if (data.startsWith('fb_edit:')) {
                ...
                if (targetPath) {
                    ...
                    await ctx.answerCallbackQuery();
                    await ctx.reply(`✏️ *Editing file:* \`${path.basename(targetPath)}\`\n\n` +
                        `Отправьте новое содержимое файла обычным сообщением. Файл на ПК будет полностью перезаписан.`, { parse_mode: 'Markdown' });
                }
                else {
                    await ctx.answerCallbackQuery({ text: '⚠️ Path expired or not found.' });
                }
        ...
        this.bot.on('message:voice', async (ctx) => {
            ...
            try {
                ...
                const resJson = await transcriptionResponse.json();
                const text = resJson.candidates?.[0]?.content?.parts?.[0]?.text?.trim();
                if (!text) {
                    throw new Error('Gemini API returned an empty transcription.');
                }
                const deliveredToPendingRequest = this.resolveIncomingResponse(text, ctx.message.reply_to_message?.message_id, 'text');
                if (deliveredToPendingRequest) {
                    await this.bot.api.editMessageText(ctx.chat.id, loadingMsg.message_id, `🎙️ *Voice response sent to agent:* \n\n_"${text}"_`, { parse_mode: 'Markdown' });
                    return;
                }
                const task = this.createIncomingTask({
                    kind: 'voice',
                ...
                await this.bot.api.editMessageText(ctx.chat.id, loadingMsg.message_id, `📝 *Voice task transcribed and queued:*\n\n_"${text}"_\n\nTask: \`${task.id}\``, { parse_mode: 'Markdown' });
            }
            catch (err) {
                const errorMsg = err instanceof Error ? err.message : String(err);
                await this.bot.api.editMessageText(ctx.chat.id, loadingMsg.message_id, `⚠️ *Failed to transcribe voice:* \n\`${errorMsg}\``, { parse_mode: 'Markdown' });
            }
        });
        ...
        this.bot.on('message:document', async (ctx) => {
            ...
            try {
                ...
                const deliveredToPendingRequest = this.resolveIncomingResponse(taskText, ctx.message.reply_to_message?.message_id, 'file');
                if (deliveredToPendingRequest) {
                    await this.bot.api.editMessageText(ctx.chat.id, loadingMsg.message_id, `📁 *File received & saved!* \nAttachment details sent to agent: \`${absolutePath}\``, { parse_mode: 'Markdown' });
                    return;
                }
                const task = this.createIncomingTask({
                    kind: 'file',
                ...
                const errorMsg = err instanceof Error ? err.message : String(err);
                await this.bot.api.editMessageText(ctx.chat.id, loadingMsg.message_id, `⚠️ *Failed to download file:* \n\`${errorMsg}\``, { parse_mode: 'Markdown' });
            }
        });
    ...
    async executeRemoteCommand(cmd) {
        ...
        const initialMsg = await this.bot.api.sendMessage(this.chatId, `⏳ *Running command:* \`${cmd}\`\n\`\`\`\nStarting...\n\`\`\``, {
            parse_mode: 'Markdown',
        ...
        const interval = setInterval(async () => {
            ...
            try {
                await this.bot.api.editMessageText(this.chatId, runProc.messageId, `⏳ *Running:* \`${cmd}\`\n\`\`\`\n${logs || 'Waiting for output...'}\n\`\`\``, {
                    parse_mode: 'Markdown',
                    reply_markup: new InlineKeyboard().text('🛑 Cancel', `kill_proc:${pid}`),
                });
            }
        ...
        processInstance.on('close', async (code) => {
            ...
            try {
                await this.bot.api.editMessageText(this.chatId, runProc.messageId, `${statusIcon} *Completed:* \`${cmd}\` (Code: ${code})\n\`\`\`\n${logs || 'No output'}\n\`\`\``, { parse_mode: 'Markdown' });
            }
            catch {
                // ignore
            }
    ...
    getStatusText() {
        return (`📊 *Antigravity Remote Status*\n\n` +
            `🔗 *Connected:* ✅\n` +
            ...
            this.bot
                ...
                onStart: async (botInfo) => {
                    ...
                    try {
                        ...
                        console.error('[TG] Bot command menu cleared');
                    }
                    catch (err) {
                        console.error('[TG] Failed to clear bot commands:', err);
                    }
                    if (this.chatId) {
                ...
                if (!resolved)
                    ...
                    console.error('[TG] Polling stopped:', err);
            });
        });
    }
    async stop() {
    ...
    async ask(question, options, timeoutMs, taskId, acceptFiles = false) {
        this.ensureChatId();
        const requestId = this.uid();
        const timeout = timeoutMs ?? this.defaultTimeout;
        if (taskId)
            this.taskStore.markWaiting(taskId, question);
        let promptMessageId;
        if (options && options.length > 0) {
            ...
            const prompt = await this.bot.api.sendMessage(this.chatId, `${question}\n\n${responseHint}`, { parse_mode: 'Markdown' });
            promptMessageId = prompt.message_id;
        }
    ...
    async updateTaskProgress(taskId, text, percent) {
        ...
        return this.upsertTaskStatus(task, `⏳ Task ${task.id}${percentText}\n\n${text}`, keyboard);
    }
    async completeTask(taskId, summary, filePaths = []) {
    ...
    async failTask(taskId, error, retryable) {
        const task = this.taskStore.fail(taskId, error, retryable);
        const keyboard = new InlineKeyboard();
        if (retryable)
            keyboard.text('🔄 Retry', `task_retry:${task.id}`);
        keyboard.text('🛑 Cancel', `task_cancel:${task.id}`);
        return this.upsertTaskStatus(task, `⚠️ Task ${task.id} failed\n\n${error}`, keyboard);
    }
    checkTaskCancellation(taskId, markCancelled = false) {
    ...
    async sendFiles(filePaths, caption) {
        this.ensureChatId();
        if (filePaths.length === 0)
        ...
        for (let start = 0; start < files.length;) {
            ...
            if (group.length === 1) {
                ...
                const media = group.map((item, index) => InputMediaBuilder.photo(new InputFile(fs.readFileSync(item.absolutePath), item.filename), index === 0 && groupCaption ? { caption: groupCaption } : {}));
                await this.bot.api.sendMediaGroup(this.chatId, media);
            }
            else {
                const media = group.map((item, index) => InputMediaBuilder.document(new InputFile(fs.readFileSync(item.absolutePath), item.filename), index === 0 && groupCaption ? { caption: groupCaption } : {}));
                await this.bot.api.sendMediaGroup(this.chatId, media);
            }
            if (groupCaption)
                captionSent = true;
    ...
    async takeScreenshot(url, caption) {
    ...
    getMessages() {
        ...
        if (tasks.length > 0)
            this.taskStore.ack(tasks.map((task) => task.id));
    ...
    buildFileBrowserKeyboard(targetPath) {
        const keyboard = new InlineKeyboard();
        // Up button
    ...
    async sendFileBrowser(chatId, targetPath) {
        const keyboard = this.buildFileBrowserKeyboard(targetPath);
        await this.bot.api.sendMessage(chatId, `📂 *File Browser*\n\nPath: \`${targetPath.replace(/\\/g, '/')}\``, {
            parse_mode: 'Markdown',
            reply_markup: keyboard,
        });
    }
    async editFileBrowser(ctx, targetPath) {
        const keyboard = this.buildFileBrowserKeyboard(targetPath);
        try {
            await ctx.editMessageText(`📂 *File Browser*\n\nPath: \`${targetPath.replace(/\\/g, '/')}\``, {
                parse_mode: 'Markdown',
                reply_markup: keyboard,
            });
        }
    ...
    async showFileMenu(ctx, filePath) {
        ...
        await ctx.editMessageText(`📄 *File:* \`${path.basename(filePath)}\`\n` +
    ...
    async viewFileContent(ctx, filePath) {
        try {
    ...
    waitForResponse(requestId, timeoutMs, options) {
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                this.pendingRequests.delete(requestId);
                this.optionsMap.delete(requestId);
