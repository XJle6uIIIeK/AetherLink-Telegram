...
Версия **2.2.2** работает только локально через **stdio**. Remote MCP, OAuth, Docker и внешний HTTPS-сервер больше не нужны.
...
## Telegram без slash-команд

Бот больше не публикует и не обрабатывает пользовательские команды `/menu`, `/files`, `/status`, `/help` и `/run`. При запуске AetherLink вызывает Telegram `deleteMyCommands()`, поэтому старое меню команд очищается автоматически.

Единственный технический slash-message — одноразовый `/start <key>` из pairing-ссылки. Он используется только до первой привязки, не публикуется в меню Telegram и после pairing игнорируется.
