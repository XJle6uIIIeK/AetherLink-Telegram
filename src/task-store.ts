...
export class TaskStore {
  ...
  create(input: CreateTaskInput): TelegramTask {
    ...
    if (result.changes === 0) {
      if (!dedupeKey) throw new Error('Failed to create task');
      const existing = this.getByDedupeKey(dedupeKey);
      if (!existing) throw new Error(`Task deduplication failed for ${dedupeKey}`);
      return existing;
    }
  ...
  list(statuses: TaskStatus[] = ['new'], limit = 20): TelegramTask[] {
    ...
    const rows = this.db.prepare(`
      ...
      LIMIT ?
    `).all(...selectedStatuses, safeLimit) as unknown as TaskRow[];
  ...
  resume(id: string): TelegramTask {
    const task = this.require(id);
    if (task.status !== 'waiting_user') return task;
    const now = new Date().toISOString();
    this.db.prepare(`
      UPDATE tasks
  ...
  private require(id: string): TelegramTask {
    const task = this.get(id);
    if (!task) throw new Error(`Task not found: ${id}`);
    return task;
  }

  private getByDedupeKey(dedupeKey: string): TelegramTask | null {
    const row = this.db.prepare(
  ...
  private hydrate(row: TaskRow): TelegramTask {
  ...
  private ensureNotTerminal(task: TelegramTask): void {
    if (task.status === 'completed' || task.status === 'cancelled') {
      throw new Error(`Task ${task.id} is already ${task.status}`);
    }
  }

  private ensureActionable(task: TelegramTask): void {
  ...
  private addEvent(taskId: string, event: string, payload?: unknown): void {
    this.db.prepare(`
